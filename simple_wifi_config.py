...
# Simplified WiFi configuration server for testing
...
def start                <div class="password-container">
                    <input type="password" id="mqtt_password" name="mqtt_password" value="{current_config.get('mqtt_password', '')}">
                    <button type="button" class="eye-button" onclick="togglePassword('mqtt_password', this)">👁</button>
                </div>ple_ap():
    """Start a simple WiFi AP with basic web server"""
    ...
    while True:
        try:
            ...
            elif any(url in request for url in [
                'GET /generate_204',  # Android
                'GET /hotspot-detect.html',  # iOS
                'GET /connecttest.txt',  # Windows
                'GET /redirect',  # Common redirect
                'GET /success.txt',  # Common success check
                'GET /ncsi.txt'  # Windows NCSI
            ]):
                send_captive_redirect(conn)
            else:
                # Send configuration form for all other requests
...
<head>
    ...
    <style>
        ...
        .eye-button {{ position: absolute; right: 8px; top: 50%; transform: translateY(-50%); background: #f8f9fa; border: 1px solid #ddd; cursor: pointer; padding: 4px 8px; font-size: 12px; color: #666; border-radius: 3px; }}
        ...
        <form method="POST" action="/">
            ...
            <div class="form-group">
                ...
                <div class="password-container">
                    ...
                    <button type="button" class="eye-button" onclick="togglePassword('wifi_password', this)">👁</button>
                </div>
            </div>
