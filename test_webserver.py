...
<head>
    ...
    <script>
        ...
        function updateTimezoneFromCity() {{
            ...
            if (selectedValue) {{
                const [offset, dst] = selectedValue.split(',');
                
                // Update timezone offset field
                const timezoneField = document.getElementById('timezone_offset');
                if (timezoneField) {{
                    timezoneField.value = offset;
                }}
                
                // Update DST region field
                const dstField = document.getElementById('dst_region');
                if (dstField) {{
                    dstField.value = dst;
                }}
            ...
            <div id="advanced-content" class="advanced-content">
                <div class="section">
                    ...
                    <div class="input-group" style="display: none;">
                        <label for="timezone_offset">Timezone Offset (hours from UTC)</label>
                        <input type="number" name="timezone_offset" id="timezone_offset" value="{ntp_config['timezone_offset']}" min="-12" max="14" step="0.5">
                        <small style="color: #666; font-size: 0.85em;">Auto-populated by city selection</small>
                    </div>
                    <div class="input-group" style="display: none;">
                        <label for="dst_region">Daylight Saving Time Region</label>
                        <select name="dst_region" id="dst_region">
                            <option value="NONE" {'selected' if ntp_config.get('dst_region', 'NONE') == 'NONE' else ''}>No DST</option>
